...
Another POMDP planning framework which tries to explicitly decouple exploration and exploitation is [active inference](https://arxiv.org/abs/2001.07203). Active inference can be seen as defining a reward function on agent beliefs which is the sum of a pragmatic and an epistemic term (see my previous posts on [the various ways this function can be defined](https://rw422scarlet.github.io/2023/07/30/active-inference-introduction.html) and [its motivation and origin](https://rw422scarlet.github.io/2023/09/07/rationalize-efe.html)). Thus, given an action sequence, one can easily query its pragmatic and epistemic value separately. However, given the vanilla active inference objective does not take into account counterfactual future beliefs but rather future states, it relies heavily on the environment to work in favor of the agent, namely the epistemic term drives the agent to rapidly resolve all uncertainties and only then switch to the exploitation mode. More recent [sophisticated active inference](https://arxiv.org/abs/2006.04120) solves this problem by taking into account future beliefs, but ends up facing the same problem: the entanglement of epistemic and pragmatic values. 
