...
$$
\begin{align}
\nabla_{\theta}L(\theta) &= \nabla_{\theta}\log \hat{P}_{\theta}(\tau_a) - \mathbb{E}_{\overline{P}(c=\tau|s, \theta)}\left[\nabla_{\theta}\log \hat{P}_{\theta}(\tau)\right] \\
&= \nabla_{\theta}\log \hat{P}_{\theta}(\tau_a) - \left[\overline{P}(c=\tau_a|s; \theta)\nabla_{\theta}\log \hat{P}_{\theta}(\tau_a) + \overline{P}(c=\tau_b|s; \theta)\nabla_{\theta}\log \hat{P}_{\theta}(\tau_b)\right] \\
&= \overline{P}(c=\tau_b|s; \theta)\left[\nabla_{\theta}\log \hat{P}_{\theta}(\tau_a) - \nabla_{\theta}\log \hat{P}_{\theta}(\tau_b)\right]
\end{align}
$$
</center>
where the overline $$\overline{P}$$ denotes stop gradient. Here, $$\overline{P}(c|s; \theta)$$ is the model's current choice over the two trajectories. 
